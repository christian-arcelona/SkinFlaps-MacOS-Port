...
int PDTetSolver<T, d>::addSuture(const int(&tets)[2], const T(&barycentricWeights)[2][d], const T stiffness)
{
	...
	typename DeformerType::Constraint c1; typename DeformerType::Constraint c2;
...
void PDTetSolver<T, d>::solve()
{
	...
	if (hasCollision) {
		...
		m_solver_c.updatePardiso(m_gridDeformer.m_collisionConstraints, m_gridDeformer.m_collisionSutures);
			...
			// The three coordinate solves share one read-only factor and are
			// mutually independent, so they run concurrently, each with its own
			// rhs/x buffer and cholmod workspace.
			{
				auto& pw = m_solver_c.m_pardiso;
				if (pw.m > 0 && pw.eigen_schur_initialized) {
					const int n = static_cast<int>(pw.n);
					std::array<std::vector<T>, d> rhsv, xv;
					for (int v = 0; v < d; v++) {
						rhsv[v].assign(static_cast<size_t>(n), T(0));
						xv[v].assign(static_cast<size_t>(n), T(0));
						m_solver_c.copyInTo(f, v, rhsv[v].data());
					}
					std::array<std::string, d> err;
					tbb::parallel_for(0, d, [&](int v) {
						cholmod_common cc;
						cholmod_start(&cc);
						try {
							pw.solveSchurOneShot_tls(rhsv[v].data(), xv[v].data(), cc);
					...
					for (int v = 0; v < d; v++) {
						if (!err[v].empty()) throw std::runtime_error(
							"parallel coord solve (v=" + std::to_string(v) + "): " + err[v]);
						m_solver_c.copyOutFrom(delta_X, v, xv[v].data());
					}
				} else {
					for (int v = 0; v < d; v++) {
						m_solver_c.copyIn(f, v); //copyIn
...
void PDTetSolver<T, d>::initializeDeformer_multires(const int(*elements)[d + 1], const uint8_t* tetSizeMultipliers, const size_t nEls, const T gridSize)  // COURT - new version for multires tets
{
	...
	m_gridDeformer.initializeDeformer();
	{
		...
		for (int i = 0; i < nEls; i++) {
			...
			m_gridDeformer.m_elementRestVolume[i] = T(::vol) * gridSize * gridSize * gridSize *sizeMult* sizeMult* sizeMult;
		}
	}
}
