...
namespace PhysBAM {
	...
	void GridDeformerTet<std::vector<VECTOR<dataType, dim>>>::initializeAuxiliaryStructures()
	{
        ...
        auto am = [](size_t align, size_t size)->void* { return pd::aligned_malloc(align, size); };

        m_reshapeUncollisionX = reinterpret_cast<BlockedShapeMatrixType>(am(Alignment, (size_t)m_nUncollisionBlocks*BlockWidth*(d + 1)*d * sizeof(T)));
        m_reshapeCollisionX = reinterpret_cast<BlockedShapeMatrixType>(am(Alignment, (size_t)m_nCollisionBlocks*BlockWidth*(d + 1)*d * sizeof(T)));

        m_reshapeUncollisionGradientMatrix = reinterpret_cast<BlockedMatrixType>(am(Alignment, (size_t)m_nUncollisionBlocks*BlockWidth*d*d * sizeof(T)));
        m_reshapeCollisionGradientMatrix = reinterpret_cast<BlockedMatrixType>(am(Alignment, (size_t)m_nCollisionBlocks*BlockWidth*d*d * sizeof(T)));

        m_reshapeUncollisionElementRestVolume = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nUncollisionBlocks*BlockWidth * sizeof(T)));
        m_reshapeCollisionElementRestVolume = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nCollisionBlocks*BlockWidth * sizeof(T)));

        m_reshapeUncollisionMuLow = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nUncollisionBlocks * BlockWidth * sizeof(T)));
        m_reshapeCollisionMuLow = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nCollisionBlocks * BlockWidth * sizeof(T)));

        m_reshapeUncollisionMuHigh = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nUncollisionBlocks * BlockWidth * sizeof(T)));
        m_reshapeCollisionMuHigh = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nCollisionBlocks * BlockWidth * sizeof(T)));

        m_reshapeUncollisionRangeMin = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nUncollisionBlocks * BlockWidth * sizeof(T)));
        m_reshapeCollisionRangeMin = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nCollisionBlocks * BlockWidth * sizeof(T)));

        m_reshapeUncollisionRangeMax = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nUncollisionBlocks * BlockWidth * sizeof(T)));
        m_reshapeCollisionRangeMax = reinterpret_cast<BlockedScalarType>(am(Alignment, (size_t)m_nCollisionBlocks * BlockWidth * sizeof(T)));

		if ((m_nUncollisionBlocks>0 && (m_reshapeUncollisionX == nullptr || m_reshapeUncollisionGradientMatrix == nullptr || m_reshapeUncollisionElementRestVolume == nullptr)) ||
			(m_nCollisionBlocks>0 && (m_reshapeCollisionX == nullptr || m_reshapeCollisionGradientMatrix == nullptr || m_reshapeCollisionElementRestVolume == nullptr)))
        ...
        m_reshapeUncollisionElement = reinterpret_cast<BlockedElementType>(am(Alignment, (size_t)m_nUncollisionBlocks*BlockWidth*(d + 1) * sizeof(int)));
        m_reshapeCollisionElement = reinterpret_cast<BlockedElementType>(am(Alignment, (size_t)m_nCollisionBlocks*BlockWidth*(d + 1) * sizeof(int)));
...
//                x1 = x1 - x2 - (x1 - x2).Normalized()*suture.m_restLength;
    ...
    void GridDeformerTet<std::vector<VECTOR<dataType,dim>>>::addElasticForce(std::vector<VECTOR<dataType, dim>> &SIMDf, const ElementFlag flag /*, const dataType rangeMin, const dataType rangeMax, const dataType weightProportion */ ) const
    {
        ...
        if (flag == ElementFlag::unCollisionEl) {
            BlockedShapeMatrixType reshapeUncollisionf = reinterpret_cast<BlockedShapeMatrixType>(pd::aligned_malloc(Alignment, (size_t)m_nUncollisionBlocks*BlockWidth*(d+1)*d*sizeof(T)));
...
#pragma omp parallel for
                ...
                pd::aligned_free(reshapeUncollisionf);
			}
			else {
            ...
            BlockedShapeMatrixType reshapeCollisionf = reinterpret_cast<BlockedShapeMatrixType>(pd::aligned_malloc(Alignment, (size_t)m_nCollisionBlocks*BlockWidth*(d+1)*d*sizeof(T)));
...
#pragma omp parallel for
                ...
                pd::aligned_free(reshapeCollisionf);
			}
			else {
    ...
    void GridDeformerTet<std::vector<VECTOR<dataType,dim>>>::deallocateAuxiliaryStructures() {
        pd::aligned_free(m_reshapeUncollisionX);
        pd::aligned_free(m_reshapeCollisionX);
        pd::aligned_free(m_reshapeUncollisionGradientMatrix);
        pd::aligned_free(m_reshapeCollisionGradientMatrix);
        pd::aligned_free(m_reshapeUncollisionElementRestVolume);
        pd::aligned_free(m_reshapeCollisionElementRestVolume);
        pd::aligned_free(m_reshapeUncollisionElement);
        pd::aligned_free(m_reshapeCollisionElement);

        pd::aligned_free(m_reshapeUncollisionMuHigh);
        pd::aligned_free(m_reshapeCollisionMuHigh);
        pd::aligned_free(m_reshapeUncollisionMuLow);
        pd::aligned_free(m_reshapeCollisionMuLow);
        pd::aligned_free(m_reshapeUncollisionRangeMin);
        pd::aligned_free(m_reshapeCollisionRangeMin);
        pd::aligned_free(m_reshapeUncollisionRangeMax);
        pd::aligned_free(m_reshapeCollisionRangeMax);
