...
public:
	...
	// Read-only view of the constraint sets, used to decide which lattice nodes
	// anything holds in place before the solver is (re)initialized.
	struct ConstraintDump {
		std::array<int, d + 1> element{};
		std::array<T, d + 1> weights{};
		std::array<T, d> target{};
		T stiffness = T(0);
		T stressLimit = T(0);
	};

	void exportConstraints(std::vector<ConstraintDump>& primary,
	                       std::vector<ConstraintDump>& fake,
	                       std::vector<ConstraintDump>& collision) const {
		primary.clear();
		fake.clear();
		collision.clear();
		auto convert = [&](const typename DeformerType::Constraint& src,
		                   std::vector<ConstraintDump>& dst) {
			ConstraintDump dump{};
			for (int i = 0; i < d + 1; ++i) {
				dump.element[i] = src.m_elementIndex[i];
				dump.weights[i] = src.m_weights[i];
			}
			for (int i = 0; i < d; ++i)
				dump.target[i] = src.m_xT(i + 1);
			dump.stiffness = src.m_stiffness;
			dump.stressLimit = src.m_stressLimit;
			dst.push_back(dump);
		};
		for (const auto& c : m_gridDeformer.m_constraints) convert(c, primary);
		for (const auto& c : m_gridDeformer.m_fakeSutures) convert(c, fake);
		for (const auto& c : m_gridDeformer.m_collisionConstraints) convert(c, collision);
	}

	void exportMuArrays(std::vector<T>& muLowOut, std::vector<T>& muHighOut) const {
		muLowOut.assign(m_gridDeformer.m_muLow.begin(), m_gridDeformer.m_muLow.end());
		muHighOut.assign(m_gridDeformer.m_muHigh.begin(), m_gridDeformer.m_muHigh.end());
	}
