...
//		initializePhysics();  // don't do this here.  Do in calling routine due to group initilization.
	...
	// Constraint sets in double precision, independent of the solver's scalar
	// type, so the scene can find lattice nodes that nothing anchors.
	struct ConstraintExport {
		std::array<int, d + 1> element{};
		std::array<double, d + 1> weights{};
		std::array<double, d> target{};
		double stiffness = 0.0;
		double stressLimit = 0.0;
	};

	inline void exportConstraintData(std::vector<ConstraintExport>& primary,
	                                 std::vector<ConstraintExport>& fake,
	                                 std::vector<ConstraintExport>& collision,
	                                 std::vector<double>& muLow,
	                                 std::vector<double>& muHigh) const {
		std::vector<typename PDTetSolver<T, d>::ConstraintDump> primaryRaw, fakeRaw, collisionRaw;
		m_solver.exportConstraints(primaryRaw, fakeRaw, collisionRaw);
		auto convert = [&](const std::vector<typename PDTetSolver<T, d>::ConstraintDump>& src,
		                   std::vector<ConstraintExport>& dst) {
			dst.clear();
			dst.reserve(src.size());
			for (const auto& c : src) {
				ConstraintExport out{};
				for (int i = 0; i < d + 1; ++i) {
					out.element[i] = c.element[i];
					out.weights[i] = static_cast<double>(c.weights[i]);
				}
				for (int i = 0; i < d; ++i)
					out.target[i] = static_cast<double>(c.target[i]);
				out.stiffness = static_cast<double>(c.stiffness);
				out.stressLimit = static_cast<double>(c.stressLimit);
				dst.push_back(out);
			}
		};
		convert(primaryRaw, primary);
		convert(fakeRaw, fake);
		convert(collisionRaw, collision);
		std::vector<T> muLowTmp, muHighTmp;
		m_solver.exportMuArrays(muLowTmp, muHighTmp);
		muLow.assign(muLowTmp.begin(), muLowTmp.end());
		muHigh.assign(muHighTmp.begin(), muHighTmp.end());
	}
