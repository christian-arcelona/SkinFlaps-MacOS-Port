...
bool deepCut::cutDeep()  // interpost connection data already loaded in _deepPosts
{
	...
	for (auto rit = rtiHits.begin(); rit != rtiHits.end(); ) {  // these must be per post pairs. Searching for interior intersected holes in interpost quads only.
		...
		assert(i0 == 1);
		++rit;
		// An odd intersection count leaves rit at end() here, so it must be
	...
	auto punchDeepVert = [&](rayTriangleIntersect& ri) ->bool{
		...
		if (mat == 2) {
			...
			if (ri.deepVert < 0) {  // no interior volume beneath this skin point, so no deep vertex could be made
				std::cout << "deep cut point has no interior volume beneath it\n";
				return false;
			}
		}
	...
	for (int i = 1; i < _deepPosts.size(); ++i) {
		...
		// A pair of posts whose surface paths never connected leaves this
		// search empty; the indexed reads below need a found entry.
		if (i0 < 0 || i1 < 0)
			throw(std::logic_error("Deep cutter could not connect two adjacent posts.\n"));
	...
	for (auto rit = rtiHits.begin(); rit != rtiHits.end(); ++rit) {  // these must be pairs searching for interior intersected holes
		...
		int hits = rit->second;
		++rit;
		if (rit == rtiHits.end() || post != rit->first.first || rit->first.second != i0 + 1 || rit->second != hits)
	...
	auto inputScl = [&](std::map<std::pair<int, int>, int >::iterator &rit) ->bool{
		...
		_chainFromPost = rit->first.first;  // for refusal messages raised inside this line
		_chainToPost = postTo;
	...
	_chainFromPost = _chainToPost = -1;  // the chain is done; lines cut from here (holes) have no post pair to name
	// now cut the quads
	...
	// The "all deep vertices" assumption above fails where the cut wall meets
	// thin skin over periosteum (the lower lid): a wall-triangle vertex on the
	// skin/periosteum border IS a skin vertex, its clone ends up in the skin
	// triangles of the other side of the cut, and a later undermine or cut
	// reaching it dereferences a missing map entry.  Give each cloned skin
	// vertex a record: the source's locus with NO deep partner.  A copied
	// partner can be a vertex the next excise removes (the clones ARE the
	// other side of the cut) and the next deep cut's getDeepSpatialCoordinates()
	// would then read a stale tet through it; remapping it to the partner's
	// own clone faults in the excise (the duplicated deep vertices are wall
	// vertices, not bed partners).  -1 is the loader's own "nothing to
	// undermine beneath" state, handled by every consumer, and at a
	// skin/periosteum border it is the truth.  Interior vertices have no
	// record and are untouched.
	for (auto& osv : oppositeSideVertices) {
		if (osv.second.pos == osv.first)
			continue;  // not duplicated
		auto dbit = _deepBed.find(osv.first);
		if (dbit == _deepBed.end() || _deepBed.count(osv.second.pos))
			continue;
		deepPoint dp = dbit->second;
		dp.deepMtVertex = -1;
		_deepBed.insert(std::make_pair(osv.second.pos, dp));
	}
	...
	borderWedgesToWallPairs();  // the border wedges become incision wall pairs (see the function)
...
bool deepCut::getDeepSpatialCoordinates()
{
	...
	for (auto dbit = _deepBed.begin(); dbit != _deepBed.end(); ++dbit){  // avoid logN hash searches
		// A record whose key or deep partner no longer names a live vertex with a
		// tet (an excise removed it) would be read through a stale index below;
		// treat it as invalid for this cut.
		{
			const int key = dbit->first, partner = dbit->second.deepMtVertex;
			const int nv = _mt->numberOfVertices(), nvt = (int)_vbt->_vertexTets.size();
			bool bad = key < 0 || key >= nv;
			if (!bad && partner > -1) {
				if (partner >= nvt) bad = true;
				else { const int tet = _vbt->_vertexTets[partner]; if (tet < 0 || tet >= _vbt->tetNumber()) bad = true; }
			}
			if (bad) {
				if (key < 0 || key >= nv)
					continue;
				dbit->second.deepMtVertex = -1;  // partner gone: its deep position comes from the locus below, as for any vertex without one
			}
		}
		if (dbit->second.deepMtVertex > -1){  // mark material 3-4 vertices invalid
			std::vector<materialTriangles::neighborNode> nei;
...
bool deepCut::inputCorrectFence(fence* fp, FacialFlapsGui* ffg) {
	...
	// A cut open at exactly one end is walked from its closed post toward the
	// open one.  The chain only builds the walls the later stages expect in
	// that direction (see cutSkinLine: an open FIRST post makes the open end's
	// skin line follow a wall-less deep-surface line and the cut fails).  The
	// surgeon may place the posts in either order; the cutter takes them in
	// the order it was finished for.  The tissue result is the same set of
	// post planes either way.
	const bool reversed = (n >= 2 && startOpen && !endOpen);
	if (reversed) {
		std::reverse(positions.begin(), positions.end());
		std::reverse(normals.begin(), normals.end());
		std::reverse(triangles.begin(), triangles.end());
		for (int i = 0, j = n - 1; i < j; ++i, --j) {
			std::swap(uv[i * 2], uv[j * 2]);
			std::swap(uv[i * 2 + 1], uv[j * 2 + 1]);
		}
		std::swap(startOpen, endOpen);
	}
	auto userPost = [&](int i) { return reversed ? n - i : i + 1; };  // message numbering in the order the posts were placed
	_postsReversed = reversed;
	_fenceRefusal.clear();
	for (int i = 0; i < n; ++i) {
		...
		if (addDeepPost(triangles[i], (const float(&)[2])uv[i * 2], -Vec3d(normals[i]), closedEnd) < 0) {
			char str[200];
			sprintf(str, "Post number %d needs direction adjustment.", userPost(i));
	...
	for (int i = 1; i < n; ++i) {
		if (!topConnectToPreviousPost(i)) {
			std::string str = "Post number " + std::to_string(userPost(i)) + " has no top side connection to previous post.\n";
			if (!_fenceRefusal.empty())
				str += _fenceRefusal + "\n";
			str += "Move it or change its direction and try again-";
			ffg->sendUserMessage(str.c_str(), "Please correct deep cut-");
			return false;
		}
	}
	for (int i = 1; i < n; ++i) {
		if (!deepConnectToPreviousPost(i)) {
			std::string str = "Post number " + std::to_string(userPost(i)) + " has no bottom connection to previous post.\n";
			if (!_fenceRefusal.empty())
				str += _fenceRefusal + "\n";
			str += "Adjust its post direction or previous post direction.";
			ffg->sendUserMessage(str.c_str(), "Please correct deep cut-");
			return false;
		}
	}
	if (n > 1) {  // no corrections necessary til 2
		// The crossing tested here is between post lines of THIS cut (a crossed
		// pair spans a self-intersecting wall between them).
		for (int i = 1; i < n; ++i) {
			if (preventPreviousCrossover(i) > 0) {
				char str[200];
				sprintf(str, "Post line %d crosses another post line of this cut beneath the surface.\nAdjust either post's direction.", userPost(i));
				ffg->sendUserMessage(str, "Please correct deep cut-");
				return false;
			}
		}
...
bool deepCut::topConnectToPreviousPost(int postNum) {
	assert(postNum > 0);
	double lowV;
	// Two posts entering the tissue through the same surface triangle can never be
	// connected: the provisional strip between them meets that triangle only along
	// the segment joining the two entry points, so surfacePath's start-edge search
	// finds no crossing and THROWS ("This surfacePath() call does not have a valid
	// starting triangle") instead of reporting no connection.  Refuse up front,
	// naming the posts, and turn any other throw of this probe (it only probes,
	// nothing is cut yet) into the same designed refusal, so the posts stay in
	// place for adjustment.
	auto userPost = [&](int i) { return _postsReversed ? (int)_deepPosts.size() - i : i + 1; };
	if (_deepPosts[postNum - 1].triIntersects[0].triangle == _deepPosts[postNum].triIntersects[0].triangle) {
		_fenceRefusal = "Posts " + std::to_string(userPost(postNum - 1)) + " and " + std::to_string(userPost(postNum)) + " share a surface triangle.";
		return false;
	}
	try {
		...
		_fenceRefusal = "Surface path between posts " + std::to_string(userPost(postNum - 1)) + " and " + std::to_string(userPost(postNum)) + " failed.";
		return false;
	}
	...
	for (i = 1; i < n; i += 2) {
		for (j = 1; j < _deepPosts[postNum - 1].triIntersects.size(); j += 2) {
			try {
				...
				// A probe that throws (surfacePath's start-edge miss) becomes the
				// designed "no bottom connection" refusal at the first such pair; the
				// loop is not continued past it, so no cut that would throw is
				// completed here.
				auto userPost = [&](int k) { return _postsReversed ? (int)_deepPosts.size() - k : k + 1; };
				_fenceRefusal = "Deep surface path between posts " + std::to_string(userPost(postNum - 1)) + " and " + std::to_string(userPost(postNum)) + " failed.";
				return false;
			}
		...
		if (splitMat == 4) {  // mat 5 march to an undermine border edge. Pop back on top
			for (auto& db : _deepBed) {
				if (db.second.deepMtVertex == tr[(splitEdge + 1) % 3]) {
					...
					auto nit = nei.begin();
					while (nit != nei.end()) {
						auto nbit = _deepBed.find(nit->vertex);  // operator[] would insert an uninitialized record for a ring vertex without one
						if (nbit != _deepBed.end() && nbit->second.deepMtVertex == tr[splitEdge]) {
				...
				if (edgeIntersect()) {  //  || splitTri == endTriangle condition for open ends
					...
					if (cutPath) {
						...
						if (splitMat == 2 && prevMat > 4) {  // skin start from a boundary or periosteal edge
							// The pop below needs a recorded border-crossing edge; an empty
							// list would be read past.
							if (topTe.empty() || topParams.empty() || topUVs.empty())
								throw(std::logic_error("Deep cut failed: a post is on a skin border.\nMove that post away from the border-"));
							topTe.pop_back();
							...
							const int borderEdgeA = st[splitEdge], borderEdgeB = st[(splitEdge + 1) % 3];  // before the split rewrites the triangle
							mat2BorderVertex = _mt->splitTriangleEdge(splitTri, splitEdge, lastParam);
							addSkinEdgeDeepBedRecord(mat2BorderVertex, borderEdgeA, borderEdgeB, lastParam);
						...
						if (splitMat > 4 && prevMat == 2) {  // skin end from a boundary or periosteal edge
							// The pops below take the border edge and the last skin edge, then
							// read the edge before them: three entries are required.  A post
							// within two skin edges of the border leaves fewer, and the read
							// past the list yields an undefined triangle index.
							if (topTe.size() < 3 || topParams.size() < 3 || topUVs.size() < 3)
								throw(std::logic_error("Deep cut failed: a post is too close to a skin border.\nMove that post away from the border-"));
							...
							_vbt->gridLocusToBarycentricWeight(gridLocus, _vbt->tetCentroid(tet), bw);
							const int borderEdgeA = st[lastTe & 3], borderEdgeB = st[((lastTe & 3) + 1) % 3];  // before the split rewrites the triangle
							int newV = _mt->splitTriangleEdge(lastTe >> 2, lastTe & 3, lastParam);
							addSkinEdgeDeepBedRecord(newV, borderEdgeA, borderEdgeB, lastParam);
...
void deepCut::addSkinEdgeDeepBedRecord(int newV, int va, int vb, float param) {
	// A vertex the deep cutter splits into a SKIN edge -- the border vertices of
	// the two boundary branches of surfacePathSub, and a "deep-surface" line that
	// actually runs on skin (the posterior lid) -- gets no deep-bed record from
	// the split itself, so a later undermine or cut whose region reaches it
	// dereferences a missing map entry ("no deep bed correspondence").
	// Interpolate the endpoints' loci as createFlapTopBottomVertices does; the
	// deep partner stays -1 unless both endpoints share one -- the "nothing to
	// undermine beneath" signal the flood already handles at a skin/periosteum
	// border.
	auto a = _deepBed.find(va), b = _deepBed.find(vb);
	if (a == _deepBed.end() || b == _deepBed.end() || _deepBed.count(newV))
		return;
	deepPoint dp;
	dp.gridLocus = a->second.gridLocus * (1.0f - param) + b->second.gridLocus * param;
	dp.deepMtVertex = (a->second.deepMtVertex > -1 && a->second.deepMtVertex == b->second.deepMtVertex) ? a->second.deepMtVertex : -1;
	_deepBed.insert(std::make_pair(newV, dp));
}

void deepCut::borderWedgesToWallPairs() {
	// Where a cut leaves the skin at the periosteum border, mat2BorderSplit closes the
	// incision wall with one material-6 wedge per side:
	// [borderV, incisionTopV, bottomVertex] on the first side and [borderV', bottomVertex',
	// incisionTopV'] on the other, each with its base edge borderV-bottomVertex on that
	// side's solid cut wall.  The undermine rewires only material-3 wall PAIRS (top triangle
	// with the two top vertices at [0],[1], its bottom triangle next), so a wedge whose
	// incision top vertex the undermine doubles keeps the old bottom vertex while the wall
	// beside it takes the new one, and the surface comes back open (the undermine's
	// own topological throw, with the mesh already modified).  Give the last segment of the
	// incision the same structure as every other one: a zero-thickness bottom vertex bB
	// beneath borderV (the skin has no thickness there), the wedge as the pair's top
	// triangle, a bottom triangle [bottomVertex, bB, borderV] (first side) or
	// [bB, bottomVertex, borderV] (other side), the solid wall's base edge re-pointed from
	// borderV to bB, and the old wedge slot turned into the solid filler between bB and
	// borderV.  Every directed edge then has exactly one reverse.  Records: borderV -> bB;
	// on the other side the copied incision-top record gets that side's own bottom vertex.
	if (_borderWedges.empty())
		return;
	_mt->findAdjacentTriangles(true);
	for (auto& bw : _borderWedges) {
		const int w = bw.first;
		const bool far = bw.second;
		if (w < 0 || w >= _mt->numberOfTriangles() || _mt->triangleMaterial(w) != 6)
			continue;
		const int* wv = _mt->triangleVertices(w);
		const int* wt = _mt->triangleTextures(w);
		const int B = wv[0], T = far ? wv[2] : wv[1], bv = far ? wv[1] : wv[2];
		const int bTx = wt[0], tTx = far ? wt[2] : wt[1], bvTx = far ? wt[1] : wt[2];
		const unsigned int adj = _mt->triAdjs(w)[far ? 0 : 2];  // the wedge's base edge between borderV and bottomVertex
		const int S = adj == 3 ? -1 : (int)(adj >> 2);
		if (S < 0 || _mt->triangleMaterial(S) != 6)
			continue;
		const int* sv = _mt->triangleVertices(S);
		const int* st = _mt->triangleTextures(S);
		int sB = -1, sD = -1;
		for (int i = 0; i < 3; ++i) {
			if (sv[i] == B) sB = i;
			else if (sv[i] != bv) sD = i;
		}
		if (sB < 0 || sD < 0)
			continue;
		const int d = sv[sD], dTx = st[sD];
		// the bottom vertex beneath the border vertex: a small step down the wall toward the
		// skin's bottom vertex (5% of the way), embedded through the lattice's own edge-point
		// lookup so physics keeps it there.  Exactly coincident with B it makes the pair's
		// bottom triangle and the filler zero-area, and the bed rays' depth search (rayDepth
		// solves a linear system on every surface triangle) returns spurious hits there:
		// the cut edge quivers after an undermine.
		const int bB = _mt->addVertices(1);
		assert(bB == (int)_vbt->_vertexTets.size());
		{
			Vec3f gridLocus, bwt;
			const float param = 0.05f;
			const int tet = _vbt->parametricEdgeTet(B, bv, param, gridLocus);
			if (tet > -1) {
				_vbt->gridLocusToBarycentricWeight(gridLocus, _vbt->tetCentroid(tet), bwt);
				_vbt->_vertexTets.push_back(tet);
				_vbt->_barycentricWeights.push_back(bwt);
				Vec3f pos;
				_vbt->vertexBarycentricPosition(bB, pos);
				_mt->setVertexCoordinate(bB, pos.xyz);
			}
			else {
				_vbt->_vertexTets.push_back(_vbt->_vertexTets[B]);
				_vbt->_barycentricWeights.push_back(_vbt->_barycentricWeights[B]);
				_mt->setVertexCoordinate(bB, (const float(&)[3])*_mt->vertexCoordinate(B));
			}
		}
		const int bBTx = _mt->addTexture();
		_mt->setTexture(bBTx, (const float(&)[2])*_mt->getTexture(bTx));
		// the wall pair, top triangle first (addTriangle may reallocate: pointers re-fetched after)
		int V[3], X[3];
		if (!far) { V[0] = B; V[1] = T; V[2] = bv; X[0] = bTx; X[1] = tTx; X[2] = bvTx; }
		else { V[0] = T; V[1] = B; V[2] = bv; X[0] = tTx; X[1] = bTx; X[2] = bvTx; }
		const int top = _mt->addTriangle(V, 3, X);
		if (!far) { V[0] = bv; V[1] = bB; V[2] = B; X[0] = bvTx; X[1] = bBTx; X[2] = bTx; }
		else { V[0] = bB; V[1] = bv; V[2] = B; X[0] = bBTx; X[1] = bvTx; X[2] = bTx; }
		const int bot = _mt->addTriangle(V, 3, X);
		assert(bot == top + 1);
		// the solid wall now meets the pair's bottom edge at bB
		int* svm = _mt->triangleVertices(S);
		int* stm = _mt->triangleTextures(S);
		svm[sB] = bB;
		stm[sB] = bBTx;
		// the old wedge slot: the solid filler between bB and borderV
		int* wvm = _mt->triangleVertices(w);
		int* wtm = _mt->triangleTextures(w);
		if (!far) { wvm[0] = bB; wvm[1] = d; wvm[2] = B; wtm[0] = bBTx; wtm[1] = dTx; wtm[2] = bTx; }
		else { wvm[0] = bB; wvm[1] = B; wvm[2] = d; wtm[0] = bBTx; wtm[1] = bTx; wtm[2] = dTx; }
		// records
		auto rb = _deepBed.find(B);
		if (rb != _deepBed.end())
			rb->second.deepMtVertex = bB;
		else {
			auto rt = _deepBed.find(T);
			if (rt != _deepBed.end()) {
				deepPoint dp = rt->second;
				dp.deepMtVertex = bB;
				_deepBed.insert(std::make_pair(B, dp));
			}
		}
		if (far) {
			auto rt = _deepBed.find(T);
			if (rt != _deepBed.end())
				rt->second.deepMtVertex = bv;
		}
	}
	_borderWedges.clear();
}
...
void deepCut::mat2BorderSplit(int borderV, int borderTx, int incisionTopV) {  // At material 2 triangle edge with mat 1, or > 4 border, this split must be made.
	...
	// The opposite skin vertex is the other side of the incision at the border
	// and connects to the same bottom vertex (both triangles built below use it),
	// so it needs the same deep-bed record -- flapSurfaceSplitter gives its
	// opposite vertices exactly this copy.  Without it a later undermine or cut
	// whose region reaches this vertex dereferences a missing map entry
	// ("Undermine region vertex has no deep bed correspondence").
	_deepBed.insert(std::make_pair(oppVert, dbit->second));
	Vec3f V;
	...
	_borderWedges.push_back(std::make_pair(firstTri, false));  // converted to wall pairs at the end of cutDeep()
	_borderWedges.push_back(std::make_pair(oppTri, true));
	_mt->findAdjacentTriangles(true);
...
//		int tet = parametricMTedgeTet(tri, edge, *pit, gridLocus);
		const int edgeA = st[edge], edgeB = st[(edge + 1) % 3], triMat = _mt->triangleMaterial(tri);  // before the split rewrites the triangle
		int bedVertex = _mt->splitTriangleEdge(tri, edge, *pit);
		if (triMat == 2)  // a "deep-surface" line running on skin (posterior lid): the new skin vertex needs a record too
			addSkinEdgeDeepBedRecord(bedVertex, edgeA, edgeB, *pit);
...
void deepCut::cutSkinLine(int startV, Vec2d &startUV, int endV, Vec2d& endUV, std::vector<unsigned int>& te, std::vector<float>& params, std::vector<Vec2d>& UVs, bool Tin, bool Tout, surfaceCutLine& scl) {
	...
	// -1 is a sentinel on both sides: startV == -1 is a skin line that starts at
	// a skin border and has no deep-bed pair, _previousSkinTopEnd == -1 a previous
	// line that ended at an intersection without a skin vertex (a post's line
	// left the skin through a deeper material).  Comparing them equal would
	// promote a border-start line to a Tin and anchor its split on a bed vertex
	// that no triangle joins yet (a read past the ring vector).  A border-start
	// line starts free; only a real top vertex can be a Tin, decided below once
	// adjacency is current.
	const bool TinCandidate = (startV > -1 && startV == _previousSkinTopEnd);
	if (startV < 0) {
		// One shape the free start cannot take: a two-vertex line without a back
		// split (flapSurfaceSplitter writes out of bounds on it).  Refuse with the
		// remedy before this line inserts anything.  With Tout (a T-out / U-out
		// caller, or the loop-closing line) flapSurfaceSplitter appends the end's
		// deep vertex and the two-vertex line cuts.
		const bool backSplit = Tout || (endV == _loopSkinTopBegin);
		if (te.size() < 2 && !backSplit) {
			// The chain posts are known only while cutDeep()'s chain is being cut;
			// the hole finder runs after it with no post pair to name.
			const bool named = (_chainFromPost >= 0 && _chainToPost >= 0);
			auto userPost = [&](int i) { return _postsReversed ? (int)_deepPosts.size() - i : i + 1; };
			const int a = named ? userPost(_chainFromPost) : 0, b = named ? userPost(_chainToPost) : 0;
			if (named)
				throw(std::logic_error("Deep cut failed between posts " + std::to_string(a) + " and " + std::to_string(b) + ": too close to a skin border.\nMove post " + std::to_string(b) + " away from the border, or aim post " + std::to_string(a) + " higher-"));
			throw(std::logic_error("Deep cut failed: a line runs too close to a skin border.\nMove the nearby posts away from the border-"));
		}
	}
	...
	for (auto teit = te.begin(); teit != te.end(); ++teit) {
		...
		if (topVertex < 0 || bottomVertex < 0)
			throw(std::logic_error("Deep cutter could not create a skin split vertex along this cut line.\n"));
		topVerts.push_back(topVertex);
	...
	updateDeepSpatialCoordinates();
	if (TinCandidate) {
		// The previous line of the chain ended at this top vertex.  Tin is right
		// only when that line was a skin line, whose topDeepSplit_Sub() built the
		// material-3 wall joining startV to its deep-bed vertex: flapSurfaceSplitter()
		// takes that deep vertex as the Tin anchor and needs the pair to share a
		// triangle.  A deep-surface line cut by cutDeepSurface() ends at the same
		// top vertex (the intersection's mat2Vert) but builds no wall, so promoting
		// Tin there makes the anchor search miss and read past the ring vector,
		// which made a canthotomy depend on which post was placed first.  Mirror
		// the splitter's own success test; when it would miss, refuse here with
		// the reason and the remedy (the chain walks the open end last when the
		// open post is last, and then every skin line follows a walled one).
		// Leaving Tin off instead lets this line through but the deep-surface
		// polyline split later in cutDeep() faults on the unwalled chain, so the
		// unfinished traversal is refused rather than completed differently.
		{
			bool walled = false;
			auto dbit = _deepBed.find(startV);
			if (dbit != _deepBed.end() && dbit->second.deepMtVertex > -1 && !(*_mt->vertexFaceTriangle(startV) & 0x80000000)) {
				const int deepV = dbit->second.deepMtVertex;
				std::vector<materialTriangles::neighborNode> nei;
				_mt->getNeighbors(startV, nei);
				for (auto& n : nei) {
					if (n.vertex != deepV || n.triangle < 0)
						continue;
					const int* tr = _mt->triangleVertices(n.triangle);
					for (int j = 0; j < 3; ++j)
						if (tr[j] == deepV && tr[(j + 1) % 3] == startV) { walled = true; break; }
					if (walled)
						break;
				}
			}
			if (walled)
				...
				// The same state can also arise with every post closed, when an
				// interpost segment crosses a skin border and re-enters skin exactly
				// at the next post; word the remedy for the configuration in hand.
				// (A one-end-open cut is always walked from its closed post, so an
				// open-first traversal here means a cut open at BOTH ends.)
				const bool openEnd = !_deepPosts.front().closedEnd || !_deepPosts.back().closedEnd;
				if (openEnd)
					throw(std::logic_error("Deep cut failed: open at both ends, wrong direction.\nPlace the posts in the opposite order-"));
				throw(std::logic_error("Deep cut failed at a skin border.\nMove this post away from the border-"));
			}
		}
	}
