...
std::string skinCutUndermineTets::lastRefusal;
...
{  //  Only cuts material 2 triangles down to deep bed and creates single vertex deep cut line along with material 3 side triangles.
	...
	lastRefusal.clear();
	// Pre-check before any mutation: every interior cut point must resolve to
	// top skin. closestPoint() searches all materials, so a point placed on an
	// existing incision edge can land on a wall or bed triangle whose vertices
	// carry no deep-bed record, which the cut would then read through an invalid iterator.
	for (int n = (int)topCutPoints.size(), i = 0; i < n; ++i) {
		if ((i < 1 && _startOpen) || (i > n - 2 && _endOpen))
			continue;
		int ptri = _mt->numberOfTriangles();
		float puv[2];
		_mt->closestPoint(topCutPoints[i].xyz, ptri, puv);
		if (ptri < 0 || _mt->triangleMaterial(ptri) != 2) {
			lastRefusal = "Incision point " + std::to_string(i + 1) + " is not on skin.\nMove it onto skin-";
			return false;
		}
	}
	float uv[2];
	for (int n=topCutPoints.size(),i = 0; i < n; ++i){
		...
		if (uv[0] < -0.0001 || uv[1]<-0.0001 || uv[0] + uv[1]>1.001f) {
			// The assert is compiled out in release builds, so this point would get no
			// vertices (-1) and planeCutSurfaceLine() would later read vertexCoordinate(-1).
			// closestPoint() clamps u and v separately but never u+v, and
			// breaks an exact-distance tie by the lowest triangle index; once an
			// earlier point of this same cut has split the triangle, a later point
			// that lay in the same original triangle resolves to the shrunken parent
			// with coordinates outside it.  Walk across the exceeded edge into the
			// triangle that really holds the point (a hop or two) and cut there.
			const int tri0 = tri;
			const float uv0[2] = { uv[0], uv[1] };
			bool inside = false;
			for (int hop = 0; hop < 8; ++hop) {
				int edge;
				if (uv[0] + uv[1] > 1.001f) edge = 1;      // beyond edge v1-v2
				else if (uv[0] < -0.0001f) edge = 2;       // beyond edge v2-v0
				else if (uv[1] < -0.0001f) edge = 0;       // beyond edge v0-v1
				else { inside = true; break; }
				const unsigned int adj = _mt->triAdjs(tri)[edge];
				if (adj == 3)  // free edge: nothing on the other side
					break;
				tri = adj >> 2;
				if (!_mt->getBarycentricProjection(tri, topCutPoints[i].xyz, uv))
					break;
			}
			if (!inside || _mt->triangleMaterial(tri) != 2) {
				lastRefusal = "Incision point " + std::to_string(i + 1) + " is too close to an earlier point.\nSpace the points farther apart-";
				return false;
			}
			// closestPoint() clamps u and v to [0,1] on the direct path; the plane
			// projection does not, and createFlapTopBottomVertices() forwards v as
			// an unclamped edge-1 split parameter, so bring the accepted uv inside.
			if (uv[0] < 0.0f) uv[0] = 0.0f;
			if (uv[1] < 0.0f) uv[1] = 0.0f;
			if (uv[0] + uv[1] > 1.0f) { const float s = 1.0f / (uv[0] + uv[1]); uv[0] *= s; uv[1] *= s; }
			createFlapTopBottomVertices(tri, uv, topMtVertices[i], deepVertexLine[i]);
		}
		else {
			if (_mt->triangleMaterial(tri) != 2) {  // the surface changed under an earlier point of this same cut
				lastRefusal = "Incision point " + std::to_string(i + 1) + " is no longer on skin.\nSpace the points farther apart-";
				return false;
			}
			createFlapTopBottomVertices(tri, uv, topMtVertices[i], deepVertexLine[i]);
		}
...
{  // If uv == 0, 0 || uv == 1, 0 || uv == 0, 1 existing topVertex returned, otherwise new one created. Creates/gets a corresponding bottomVertex.
	...
	for (int i = 0; i < 3; ++i){
		...
		assert(dbit != _deepBed.end());
		// The assert is compiled out in release builds; a skin vertex with
		// no deep bed entry must fail loudly, not read a null record.
		if (dbit == _deepBed.end())
			throw(std::logic_error("Skin vertex has no deep bed correspondence for this cut.\n"));
		if (i < 1)
			mult = 1.0f - uv[0] - uv[1];
		else if (i < 2)
			mult = uv[0];
...
//		assert(topVertex == _vbt->_vertexTets.size());  No longer necessarily true with skin-border deepCut
		...
		tet = flapBottomTet(topVertex, dp.gridLocus);
		assert(tet > -1);
		if (bottomTriangle < n){
			...
			assert(nit != nei.end());
			// Release builds compile the assert out; a missed ring search
			// must surface as the recoverable dialog, not an out-of-range index.
			if (nit == nei.end() || nit->triangle < 0)
				throw(std::logic_error("Deep bed connection not found for this cut point.\n"));
...
bool skinCutUndermineTets::topDeepSplit_Sub(std::list<int> &topVerts, std::list<int> &deepVerts, bool frontSplit, bool backSplit){
	...
	for (tvit = topVerts.begin(); tvit != topVerts.end(); ++tvit) {  // fix _deepBed if incising a flap bottom
		if (oppBotVerts[i] != *dvit) {
			// Indexing _deepBed by a vertex without a record would insert one with
			// an uninitialized locus; the next undermine then places bottom vertices
			// from that locus and the next deep cut's getDeepSpatialCoordinates()
			// reads an undefined tet through it.  Only update an existing record.
			auto dbit = _deepBed.find(*tvit);
			if (dbit != _deepBed.end())
				dbit->second.deepMtVertex = oppBotVerts[i];
		}
		++dvit;
		++i;
	}
	if(_mt->findAdjacentTriangles(true))
		throw(std::logic_error("Skin incision failed with a topological error.\n"));
...
{  // divides an _mt surface along a continuous vertexCutLine into two free edge sides in places where the cutting surface is already connected to the _mt surface.
	...
	assert(nit != nei.end());
	// The assert above is compiled out in release builds, and a missed
	// search would read past the ring vector as a triangle number.  The
	// search misses when the Tin/Tout anchor vertex (a deep-bed
	// counterpart) was never sewn to this surface -- the case where no
	// interior volume exists under the skin, e.g. a deep cut into the
	// chin / inner lip.  An open ring also starts with a -1 sentinel
	// triangle that must not be indexed.
	if (nit == nei.end() || nit->triangle < 0)
		throw(std::logic_error("Deep cut failed: cut line start not connected.\nTry the posts in the opposite order, or farther from the skin border-"));
	...
	assert(tri > -1);
	// tri must name a real triangle before the indexed reads below; the
	// assert above is compiled out in release builds.
	if (tri < 0)
		throw(std::logic_error("Deep cutter found no surface triangle joining the first two points of this cut line.\n"));
	do {
		...
		if (dbit != _deepBed.end()) {  // is a flap top vertex
			...
			auto dbDeep = _deepBed.insert(std::make_pair(oppositeVertices[ovn], dbit->second));  // careful of array syntax diagnostics
			if (!dbDeep.second)
				dbDeep.first->second = dbit->second;
		}
		...
		lastV = *tvit;
		++tvit;
		while (tvit != vertexCutLine.end() && tr[(vertIdx + 1) % 3] != *tvit) {
			int at[3], ae[3];
			...
			// When this pinwheel walk crosses a free edge the adjacency
			// code is 3, which decodes as triangle 0 / edge 3, and the
			// write on the next line would corrupt unrelated triangles
			// long before the walk faults.  Stop at the first wrong step.
			if (tr[vertIdx] != lastV)
				throw(std::logic_error("Deep cutter lost the surface while splitting along this cut line.\n"));
			tr[vertIdx] = oppositeVertices[ovn];
			if (ttx[vertIdx] != lastTx) {  // crossed a texture seam
				int twoTx[2];
				twoTx[0] = txId;
...
bool skinCutUndermineTets::planeCutSurfaceLine(const int startTopV, const int endTopV, const int startDeepV, const int endDeepV, std::list<int> &newTopVerts, std::list<int> &newDeepVerts)
{
	...
	for (i = 0; i < n; ++i){
		...
		if ((j & 3) < 1)
			...
			assert(false);
		int topV, botV;
		if (_mt->triangleMaterial(j >> 2) != 2) {
			// The walk between two cut points reached a non-skin triangle: the
			// path crosses an existing incision (or an excised edge).  Building
			// flap vertices there would read a deep-bed record that does not
			// exist; refuse instead and let the caller revert the partial cut.
			lastRefusal = "Incision path crosses an existing incision.\nShift-click to T into it, or route around it-";
			return false;
		}
...
{  // return deepPoint tet number and baryweight from its grid locus
	...
	if (lt.size() < 2)
		...
		tetOut = _vbt->vertexSolidLinePath(dit->first, dit->second.gridLocus);  // can return -1 which must be handled
		if (tetOut < 0)
			int junk = dit->first;
	}
	_vbt->gridLocusToBarycentricWeight(dit->second.gridLocus, tc, baryWeight);
...
int skinCutUndermineTets::flapBottomTet(const int topVertex, const Vec3f &bottomGridLocus)
{  // material coord flap bottom tet finder. Return -1 signals error in deep bed data input
	...
	int topTet = _vbt->_vertexTets[topVertex];
	...
	if (lt.size() < 2)
		...
		tetOut = _vbt->vertexSolidLinePath(topVertex, bottomGridLocus);
		assert(tetOut > -1);
	}
	return tetOut;
}
...
void skinCutUndermineTets::collectOldUndermineData()
{
	...
	for (int n = _mt->numberOfTriangles(), j, i = 0; i < n; ++i) {
		...
		if (nit->vertex == deepV[1]) {
			bot4.push_back(std::make_pair(nit->triangle, i));
...
void skinCutUndermineTets::undermineSkin() {
	...
	// true when a top vertex lies on the skin's material border (a periphery or
	// periosteum triangle in its ring); such a vertex has nothing beneath it to separate from.
	auto onSkinBorder = [&](int v) -> bool {
		std::vector<materialTriangles::neighborNode> nei;
		_mt->getNeighbors(v, nei);
		for (auto& nn : nei) {
			if (nn.triangle < 0) continue;
			const int m = _mt->triangleMaterial(nn.triangle);
			if (m == 1 || m == 7) return true;
		}
		return false;
	};
	// A top vertex the deep cutter left on a material-6 wall -- the wedge mat2BorderSplit
	// builds where a cut leaves the skin at the periosteum border -- cannot be doubled: the
	// wedge keeps the old bottom vertex while the incision wall is rewired to the new one,
	// and the surface comes back open.  Letting the undermine run reaches those open edges
	// and the topological throw at the end with the mesh already modified, so refuse before
	// anything is built.  Perimeter vertices stay single (bed and flap bottom share them)
	// and undermine fine.
	{
		std::set<int> single, seen;
		...
		for (auto& t : undTris) {
			...
			fresh.push_back(t);
			for (int k = 0; k < 3; ++k) {
				const int a = _mt->triAdjs(t)[k] >> 2;
				const int am = _mt->triangleMaterial(a);
				if ((am == 2 && !std::binary_search(undTris.begin(), undTris.end(), a)) || am == 1 || am == 7) {  // same rule as oneDeepV below
					single.insert(_mt->triangleVertices(t)[k]);
					single.insert(_mt->triangleVertices(t)[(k + 1) % 3]);
				}
			}
		}
		for (auto t : fresh) {
			const int* tv = _mt->triangleVertices(t);
			for (int j = 0; j < 3; ++j) {
				const int v = tv[j];
				if (single.count(v) || !seen.insert(v).second)
					continue;
				if (onSkinBorder(v))
					continue;
				std::vector<materialTriangles::neighborNode> nei;
				_mt->getNeighbors(v, nei);
				for (auto& nn : nei) {
					if (nn.triangle < 0 || _mt->triangleMaterial(nn.triangle) != 6)
						continue;
					throw(std::logic_error("Undermine region reaches the end of a deep cut at the skin border.  Undermine short of that end, or in two parts-\n"));
				}
			}
		}
	}
	// the bottom vertex the incision wall beneath a top vertex actually holds
	// (the pair convention: top triangle first with the two tops at [0] and [1], its
	// bottom triangle next with their bottoms at [1] and [0]).  -1 when no wall is there.
	auto wallBottomBeneath = [&](int topV) -> int {
		std::vector<materialTriangles::neighborNode> nei;
		_mt->getNeighbors(topV, nei);
		const int nt = _mt->numberOfTriangles(), nv = _mt->numberOfVertices();
		for (auto& nn : nei) {
			if (nn.triangle < 0 || nn.triangle + 1 >= nt || _mt->triangleMaterial(nn.triangle) != 3 || _mt->triangleMaterial(nn.triangle + 1) != 3)
				continue;
			const int* wt = _mt->triangleVertices(nn.triangle);
			const int idx = wt[0] == topV ? 0 : (wt[1] == topV ? 1 : -1);
			if (idx < 0)
				continue;  // a pair's bottom triangle holds its top vertex at [2]
			const int* wb = _mt->triangleVertices(nn.triangle + 1);
			const int b = idx == 0 ? wb[1] : wb[0];
			if (b > -1 && b < nv && b != topV)
				return b;
		}
		return -1;
		...
		if (!std::binary_search(_prevUnd2.begin(), _prevUnd2.end(), t)) {
			...
			for (int j = 0; j < 3; ++j) {
				...
				if (pr.second) {
					...
					// The matching assert sits after this dereference and
					// vanishes in release builds; a miss must surface as the
					// recoverable dialog, not a memory fault.
					if (dit == _deepBed.end())
						throw(std::logic_error("Undermine region vertex has no deep bed correspondence.\n"));
					int dv = dit->second.deepMtVertex;
					if (dv > -1) {
						// a partner an excise already removed (tet -2, face deleted) must not be
						// woven into the new flap bottom.
						const bool inRange = dv < (int)_vbt->_vertexTets.size() && dv < _mt->numberOfVertices();
						const bool dead = !inRange || _vbt->getVertexTetrahedron(dv) < 0 || (*_mt->vertexFaceTriangle(dv) & 0x80000000);
						if (dead)
							dv = -1;
					}
					if (dv < 0) {
						// no live partner -- the loader's "nothing beneath", a cut-border vertex whose
						// record holds none, or a partner an excise removed.  When an incision wall
						// stands beneath the vertex its bottom vertex IS the partner (what
						// createFlapTopBottomVertices records for every other cut vertex); a fresh bed
						// vertex from the locus there would leave the wall and the solid cut wall unmatched.
						const int wb = wallBottomBeneath(top[j]);
						dit->second.deepMtVertex = dv = wb;  // -1 when no wall: a fresh bed vertex below
					}
					if (dv > -1)
						pr.first->second.deepV = dv;
					else {
	...
	// The perimeter test runs over EVERY undermined triangle, not only the new ones -- a
	// vertex of a new triangle whose perimeter edge lies in a triangle undermined earlier
	// is still on the region's edge and must stay single; doubling it orphans that earlier
	// bed edge (the topological throw at the end on a re-undermine).  New triangles alone
	// decide the walls to rewire.
	for (auto &tri : undTris) {
		const bool isNew = std::binary_search(newTris.begin(), newTris.end(), tri);
		for (int k = 0; k < 3; ++k) {
			int t = (_mt->triAdjs(tri)[k] >> 2);
			const int tm = _mt->triangleMaterial(t);
			// A top vertex on the skin's material border (the edge meets periphery or
			// periosteum, materials 1 and 7) has no thickness to separate from and stays
			// single as well; doubling it leaves the bed and flap bottom edges along the
			// border unmatched.
			if ((tm == 2 && !std::binary_search(undTris.begin(), undTris.end(), t)) || tm == 1 || tm == 7) {
				oneDeepV.insert(_mt->triangleVertices(tri)[k]);
				oneDeepV.insert(_mt->triangleVertices(tri)[(k + 1) % 3]);
			}
			if (isNew && _mt->triangleMaterial(t) == 3)  // there are undermined edge tris
				underminedEdgeTris.push_back(t);  // only need top one
		}
	}
	// The border is a property of the vertex, not of the new triangles -- a vertex
	// anywhere on the skin's material border stays single in every undermine that
	// reaches it, including a later one over already undermined skin.
	for (auto& uv : undV) {
		if (oneDeepV.count(uv.first)) continue;
		if (onSkinBorder(uv.first)) oneDeepV.insert(uv.first);
	}
	...
	std::multimap<int, botD> bottomDoubles;
	for (auto& uv : undV) {
		if (oneDeepV.find(uv.first) == oneDeepV.end()) {  // should be doubled
			...
			// if a bottomDouble is at an incision edge, need top vertex to determine which side it connects to. Otherwise -1 for simple case.
			auto pr = bottomDoubles.equal_range(oldV);
			while (pr.first != pr.second) {
				if (pr.first->second.topV == uv.first)
					break;
				++pr.first;
			}
			if (pr.first == pr.second) {
				...
				bd.topV = uv.first;
				bd.dvp = &uv.second;
				bottomDoubles.insert(std::make_pair(oldV, bd));  // need these to fix old border tris from a previous undermine
			}
		}
	}
	for (auto nt : newTris) {
		...
		for (int i = 0; i < 3; ++i) {
			auto dvIt = undV.find(top[i]);  // a miss here must not be a memory fault
			if (dvIt == undV.end())
				throw(std::logic_error("Undermined surface vertex missing from the new bed.\n"));
			auto dv = dvIt->second;
	...
	auto tri4vTopConnects = [&](int tri4TopTri, botD *bd) ->bool {
		auto tr = _mt->triangleVertices(tri4TopTri);
		for (int i = 0; i < 3; ++i) {
			if(tr[i] == bd->topV)
				return true;
		}
	...
	for (auto b4 : _prevBot4) {
		int* tr = _mt->triangleVertices(b4.first);
		int* ttx = _mt->triangleTextures(b4.first);
		for (int j, i = 0; i < 3; ++i) {
			auto pr = bottomDoubles.equal_range(tr[i]);
			if (pr.first != pr.second) {
				if (std::distance(pr.first, pr.second) > 1) {
					// this is an incision edge double. Check for connection to topV
					while (pr.first != pr.second) {
						if (tri4vTopConnects(b4.second, &pr.first->second))
							break;
						++pr.first;
					}
					if (pr.first == pr.second)
				...
				tr[i] = pr.first->second.dvp->deepV;
				// texture seams are a terrible pain
				int pt = findPrevTexture(ttx[i], pr.first->second.dvp->deepTx);
				if (pt < 0) {
					pt = cloneTexture(ttx[i]);
					pr.first->second.dvp->deepTx.push_back(pt);
				}
				ttx[i] = pt;
			}
		}
	}
	for (auto& topT : underminedEdgeTris) {
	...
	auto edgeTopConnects = [&](int idx, int edgeTri, botD* bd) ->bool {
		// from incision convention dictates this
		int* topV;
		if (idx < 2)  // must be a bottom edge
			topV = _mt->triangleVertices(edgeTri - 1);
		else
			topV = _mt->triangleVertices(edgeTri);
		if (topV[0] == bd->topV || topV[1] == bd->topV)
			return true;
		return false;
		};
	for (auto tri : _prevEdge3) {
		...
		for (int j, i = 0; i < 3; ++i) {
			...
			if (pr.first != pr.second) {
				if (std::distance(pr.first, pr.second) > 1) {
					// this is an incision edge double. Check for connection to topV
					while (pr.first != pr.second) {
						if (edgeTopConnects(i, tri, &pr.first->second))
							break;
						++pr.first;
					}
					if (pr.first == pr.second)
				...
				tr[i] = pr.first->second.dvp->deepV;
				// leave fat textures as they are
			}
		}
	}
	if(_mt->findAdjacentTriangles(true))
		throw(std::logic_error("Undermine operation failed with a topological error.\n"));
...
{  // find the correct side of incision from usually 2 choices; 1 if part of a moved flap
	...
	auto deepEdgeTextureFix = [&](int edgeTriangle, bool isV0) {
		...
		int newTx = _mt->addTexture();
		_mt->setTexture(newTx, (const float(&)[2]) * oldTx);
		*deepTx = newTx;
		if ((_mt->triAdjs(edgeTriangle)[1] >> 2) == edgeTriangle + 1) {  // 1-2 quad tesselation
			if (isV0)
				_mt->triangleTextures(edgeTriangle)[2] = newTx;
		}
		else {
	...
	if (minParam < 0.001f) {
		deepEdgeTextureFix(minTri, true);
		return _mt->triangleVertices(minTri)[0];
	}
	if (minParam > 0.999f) {
		deepEdgeTextureFix(minTri, false);
...
{  // from incision convention we know
	...
	auto oneVertexT = [&](int vId) ->int {
		...
		assert(nit != nei.end());
		// The assert is compiled out here as well: a missed ring search must throw, not
		// index with an end iterator's contents.
		if (nit == nei.end() || nit->triangle < 0)
			throw(std::logic_error("Undermined surface connection not found at this point.\n"));
	...
	int* st = _mt->triangleVertices(edgeTriangle);
	int tet = _vbt->parametricEdgeTet(st[0], st[1], edgeParam, gridLocus);
//	int tet = parametricMTedgeTet(edgeTriangle, 0, edgeParam, gridLocus);
	...
	st = _mt->triangleVertices(edgeTriangle + 1);
	tet = _vbt->parametricEdgeTet(st[0], st[1], 1.0f - edgeParam, gridLocus);
	_vbt->gridLocusToBarycentricWeight(gridLocus, _vbt->tetCentroid(tet), baryWeight);
	...
	if (layers > 1) {
		...
		int* st = _mt->triangleVertices(tris[0][1]);
		tet = _vbt->parametricEdgeTet(st[0], st[1], 1.0f - edgeParam, gridLocus);
		_vbt->gridLocusToBarycentricWeight(gridLocus, _vbt->tetCentroid(tet), baryWeight);
	...
	if (notUndermined) {  // top skin subpatch not yet undermined. Undermine this subpatch then excise
		...
		for (int n = _mt->numberOfTriangles(), i = 0; i < n; ++i) {
			if (xTris[i]) {
				int *tr = _mt->triangleVertices(i);
				for (int j = 0; j < 3; ++j) {
					_vbt->setVertexTetrahedron(tr[j], -2);  // physics marked deleted
