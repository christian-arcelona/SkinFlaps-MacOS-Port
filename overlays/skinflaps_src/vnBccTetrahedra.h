...
public:
	...
	inline int vertexTetCount() const { return (int)_vertexTets.size(); }
	...
	// copies enabled so bccTetScene can snapshot and restore the lattice (undo)
	vnBccTetrahedra(const vnBccTetrahedra&) = default;
	vnBccTetrahedra& operator=(const vnBccTetrahedra&) = default;
	vnBccTetrahedra();
	~vnBccTetrahedra();
