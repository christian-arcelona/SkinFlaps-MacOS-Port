...
public:
	...
	// reconcile registry and graphics to a saved copy; physics constraints
	// are re-added by the caller's lattice rebuild.
	void restoreState(const sutures& saved);
	...
	bool laySutureLine(int suture2);  // creates a line of sutures between 2 input sutures; false when the row could not be laid intact
...
private:
	...
	static bool s_layingLine;  // deleteSuture must not wait on the physics flag its caller holds while a row is being laid
