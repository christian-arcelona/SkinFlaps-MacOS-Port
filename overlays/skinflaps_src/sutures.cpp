...
bool sutures::s_layingLine = false;
...
void sutures::restoreState(const sutures& saved)
{
	// drop live sutures the saved registry does not have
	for (auto it = _sutures.begin(); it != _sutures.end(); ) {
		if (saved._sutures.find(it->first) == saved._sutures.end()) {
			if (it->second.getSphereShape()) _shapes->deleteShape(it->second.getSphereShape());
			if (it->second.getCylinderShape()) _shapes->deleteShape(it->second.getCylinderShape());
			it = _sutures.erase(it);
		}
		else
			++it;
	}
	for (auto& sv : saved._sutures) {
		auto it = _sutures.find(sv.first);
		if (it == _sutures.end()) {
			// recreate the graphics exactly as addSuture() does
			it = _sutures.insert(std::make_pair(sv.first, sv.second)).first;
			char name[12];
			sprintf(name, "S_%u", sv.first);
			std::shared_ptr<sceneNode> sph = _shapes->addShape(sceneNode::nodeType::SPHERE, name);
			std::shared_ptr<sceneNode> cyl = _shapes->addShape(sceneNode::nodeType::CYLINDER, name);
			it->second.setSphereShape(sph);
			it->second.setCylinderShape(cyl);
			GLfloat sSize = _sutureSize;
			if (it->second._type < 2)
				sSize *= 2.0f;
			GLfloat v[3] = { 0.0f, 0.0f, 0.0f };
			if (it->second._tri && it->second._tris[0] > -1) {
				int* tv = it->second._tri->triangleVertices(it->second._tris[0]);
				GLfloat* v0 = it->second._tri->vertexCoordinate(tv[it->second._edges[0]]);
				GLfloat* v1 = it->second._tri->vertexCoordinate(tv[(it->second._edges[0] + 1) % 3]);
				for (int i = 0; i < 3; ++i)
					v[i] = v1[i] * it->second._params[0] + v0[i] * (1.0f - it->second._params[0]);
			}
			GLfloat* mm = sph->getModelViewMatrix();
			loadIdentity4x4(mm);
			scaleMatrix4x4(mm, sSize, sSize, sSize);
			translateMatrix4x4(mm, v[0], v[1], v[2]);
			mm = cyl->getModelViewMatrix();
			loadIdentity4x4(mm);
			scaleMatrix4x4(mm, sSize * 0.5f, sSize * 0.5f, sSize * 0.5f);
			translateMatrix4x4(mm, v[0], v[1], v[2]);
		}
		else {
			std::shared_ptr<sceneNode> sph = it->second.getSphereShape(), cyl = it->second.getCylinderShape();
			it->second = sv.second;
			it->second.setSphereShape(sph);
			it->second.setCylinderShape(cyl);
		}
		it->second._constraintId = -1;  // re-added by updateSuturePhysics after the lattice rebuild
		GLfloat (&col)[4] = it->second._selected ? _selectedColor : (it->second._type > 1 ? _unselectedColor : _userColor);
		if (it->second.getSphereShape()) it->second.getSphereShape()->setColor(col);
		if (it->second.getCylinderShape()) it->second.getCylinderShape()->setColor(col);
	}
	_userSutures = saved._userSutures;
	_sutureNow = saved._sutureNow;
	_userSutureNext = saved._userSutureNext;
	updateSutureGraphics();
}
...
int sutures::addSuture(materialTriangles *tri, int triangle0, int edge0, float param0)
{
	...
	// The sutureTets constructor does not initialize _constraintId, and the
	// rollback paths (linked-suture material rejection, tap-without-drag)
	// reach deleteSuture before the physics assigns a real id — its
	// "_constraintId > -1" guard would then hand uninitialized memory to
	// pdTetPhysics::deleteSuture. -1 is the sentinel the rest of this file
	// uses for "no constraint yet".
	hpr.first->second._constraintId = -1;
	char name[6];
...
int sutures::deleteSuture(int sutureNumber)
{
	...
	sit2 = sit;
	if (autoSuture) {
		// The backward walk assumes a well-formed row (autos preceded by
		// their cardinal); a malformed row must not decrement past begin().
		while (sit2->second._type > 1) {
			delSut();
			if (sit2 == _sutures.begin())
				break;
			--sit2;
		}
		assert(sit2->second._type == 1);
		if (sit2->second._type == 1) {
			sit2->second._type = 0;
			ret = sit2->first;
		}
	}
	...
	// Bounded wait: when this delete runs inside a rollback whose caller
	// has already taken the physics flag down and is itself blocking, the
	// flag can never rise and an unbounded wait freezes the interface.
	// Normal deletes clear in well under a second; fail loudly past 15 s.
	// A delete reached from laySutureLine must not wait at all: that caller
	// holds the physics flag down itself around a synchronous call, so
	// nothing could ever raise it.
	int waitedMs = 0;
	while (!s_layingLine && !_surgAct->physicsDone) {
		std::this_thread::sleep_for(std::chrono::milliseconds(20));
		waitedMs += 20;
		if (waitedMs > 15000)
			throw(std::logic_error("Suture removal timed out waiting for the physics update.\n"));
	}
	_ptp->initializePhysics();
	return ret;
}

bool sutures::laySutureLine(int suture2)
{  // creates a line of sutures between 2 sequential user input sutures
	struct layingGuard { layingGuard() { s_layingLine = true; } ~layingGuard() { s_layingLine = false; } } guard;
	if (suture2 < 1)
		return false;
	...
	for (int i = 1; i < subdivs; ++i){
		...
		while (j1 < n1){
			if (lp1 + edgeLengths1[j1] > d1 * i){
				...
				if (sutNum < 0)
					return false;  // no first-side suture was created for this step
				if (setSecondEdge(sutNum, mt, triEdges1[j1] >> 2, triEdges1[j1] & 3, param) != 0) {  // don't init physics. Do once as group
					// setSecondEdge rejected the intermediate suture and, through
					// deleteSuture, tore the row down and unlinked the cardinal:
					// stop here rather than laying orphans on a dismantled row.
					return false;
				}
				break;
			}
	...
	auto chk = _sutures.find(suture2);
	return chk != _sutures.end() && chk->second._type == 1;
}
