...
public:
	...
	bool saveFullHistoryTrail(const char *fullFilePath);  // the history including undone actions, with an undo/redo/revert record where each restore happened
	bool recordInFlightHookMove();  // a hook drag that fails mid-drag is written into the error files as the moveHook it was about to become
	bool recordInFlightSuture();    // a suture that fails at its release, with both points placed, is written as the addSuture it was about to become
	// Whole-state undo/redo: every forward move records the complete solving
	// state at its start; Cmd+Z restores the last record, Cmd+Shift+Z re-applies
	// the last undone one. Defined in surgicalActions.cpp.
	struct UndoSnapshot;
	void snapshotForwardMove(const char* what, size_t historyPos = (size_t)-1);
	void noteActionClock(const char* what);  // [action-clock] line + sidecar record
	UndoSnapshot captureSnapshot(const char* what, size_t historyPos);
	void restoreSnapshot(const UndoSnapshot& snap);
	void undoRedo(int key);
	void undoRedoSteps(int key, int steps);                       // several undo/redo moves as one gesture (Edit menu)
	std::vector<std::string> undoActionNames(int maxCount) const;  // moves that Cmd+Z would take back, newest first
	std::vector<std::string> redoActionNames(int maxCount) const;  // moves that Cmd+Shift+Z would restore, newest first
	...
	std::atomic<bool> taskThreadErrorHoldable{false};  // the failed task followed a forward move whose snapshot can restore the pre-action state
