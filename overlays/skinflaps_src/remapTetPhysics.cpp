	...
	auto processTet = [&](int tetIdx) ->bool {
		...
		if (std::distance(pr.first, pr.second) > 1) {  // origin was virtual noded level 1 tet.
			...
			if (j < 4) { // prior one to one correspondence found
				float dsq, minD = std::numeric_limits<float>::max();
				int bestTet = -1;
			...
			if (ntl == _newVnTetTris.end()) {
				// A virtual-noded fragment containing no surface triangles has
				// nothing to identify which prior fragment it continues, and
				// which otherwise ends the session ("Program error 3").
				// The neighboring no-overlap case already resolves by taking
				// the least satisfying choice; do the equivalent here — prefer
				// a prior fragment that was also surface-less, else the first
				// candidate. The assigned node positions are relaxed by the
				// next solve; as with the least satisfying choice, no
				// correspondence is recorded.
				int chosen = pr.first->second;
				for (auto prS = pr.first; prS != pr.second; ++prS) {
					if (_oldVnTetTris.find(prS->second) == _oldVnTetTris.end()) {
						chosen = prS->second;
						break;
					}
				}
				auto& oN = _oldTets[chosen];
				for (int k = 0; k < 4; ++k) {
					if (nodes[tn[k]])
						continue;
					newVnbt->_nodeSpatialCoords[tn[k]] = _oldNodePositions[oN[k]];
