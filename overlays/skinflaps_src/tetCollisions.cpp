...
void tetCollisions::initSoftCollisions(materialTriangles* mt, vnBccTetrahedra* vnt) {
	...
	// the bed triangles, for the penetration test in findSoftCollisionPairs
	_bedTris.clear();
	for (int n = _mt->numberOfTriangles(), i = 0; i < n; ++i)
		if (_mt->triangleMaterial(i) == 5)
			_bedTris.push_back(i);
	...
	tbb::parallel_for(tbb::blocked_range<size_t>(0, _bedRays.size()),
		[&](const tbb::blocked_range<size_t>& r) {
			for (size_t j = r.begin(); j != r.end(); ++j) {
				...
				int nearV = -1;
				// A hit counts as a penetration only when the hit point lies below the bed
				// surface near it (bed patches within the ray's own length).  The flap bottom
				// coincides with the bed at rest, and where the bed bends sharply (a deep cut's
				// shallow bottoms beside the loaded deeper bed) a ray cast into the bed passes
				// under the bend and comes out through the flap's own mirror a few patches
				// away; read as a penetration, that pushes the flap every solve and it quivers
				// along the deep-cut edge after an undermine.
				const float nearScale = b.N.length();  // the ray's own length: bed patches farther than this from the hit point are not local
				for (size_t n = _flapBotTris.size(), i = 0; i < n; ++i) {
					if (bedBox.Intersection(flapBox[i])) {
						...
						if (R[0] < 1e-6f || R[1] < 1e-6f || R[2] < 1e-4f || R[0] + R[1] > 1.0f || R[0] > 1.0f || R[1] > 1.0f || R[2] > 1.0f)  // R[2] determines how deep the collision must go before processing triggered. Bigger makes less sticky.
							continue;
						{
							const Vec3f Q = b.P - b.N * R[2];  // the hit point on the flap triangle
							bool onBed = false;
							for (int lt : _bedTris) {
								const int* bt = _mt->triangleVertices(lt);
								Vec3f A, B3, C3;
								_mt->getVertexCoordinate(bt[0], A.xyz); _mt->getVertexCoordinate(bt[1], B3.xyz); _mt->getVertexCoordinate(bt[2], C3.xyz);
								// bounding-box prefilter, expanded by the ray length
								if (Q.X < std::min(A.X, std::min(B3.X, C3.X)) - nearScale || Q.X > std::max(A.X, std::max(B3.X, C3.X)) + nearScale ||
									Q.Y < std::min(A.Y, std::min(B3.Y, C3.Y)) - nearScale || Q.Y > std::max(A.Y, std::max(B3.Y, C3.Y)) + nearScale ||
									Q.Z < std::min(A.Z, std::min(B3.Z, C3.Z)) - nearScale || Q.Z > std::max(A.Z, std::max(B3.Z, C3.Z)) + nearScale)
									continue;
								Vec3f e1 = B3 - A, e2 = C3 - A, nrm = e1 ^ e2;
								const float len = nrm.length();
								if (len < 1e-12f) continue;
								nrm *= 1.0f / len;
								Mat3x3f Mb(e1, e2, nrm);
								const Vec3f Rb = Mb.Robust_Solve_Linear_System(Q - A);
								if (Rb[0] < -0.05f || Rb[1] < -0.05f || Rb[0] + Rb[1] > 1.05f) continue;  // not over this patch
								if (Rb[2] > -2e-3f && Rb[2] < nearScale) {  // on or above a nearby bed patch: the bed's mirror (or a lifted flap seen through a bend), not a penetration
									onBed = true;
									break;
								}
							}
