...
public:
	...
	deepCut(const deepCut&) = default;  // copyable for whole-state snapshots (undo)
	deepCut& operator=(const deepCut&) = default;
	~deepCut(){}
...
//		std::vector<materialTriangles::matTriangle> quadTriangles;
	...
	int _chainFromPost = -1, _chainToPost = -1;  // the posts the line being cut runs between, named by refusal messages
	bool _postsReversed = false;      // inputCorrectFence walked the posts in reverse (one-end-open cut placed open-first)
	std::string _fenceRefusal;        // reason stored by a failed inter-post probe, appended to inputCorrectFence's message
	void addSkinEdgeDeepBedRecord(int newV, int va, int vb, float param);  // deep-bed record for a vertex the cutter split into a skin edge
	std::list<std::list<int> > _holePolyLines;  // pair first is deepVert, second topVert
	// the material-6 border wedges mat2BorderSplit builds where a cut leaves the skin
	// at the periosteum border -- (triangle, far side) -- converted at the end of
	// cutDeep() into incision wall pairs the undermine can rewire.
	std::vector<std::pair<int, bool> > _borderWedges;
	void borderWedgesToWallPairs();
