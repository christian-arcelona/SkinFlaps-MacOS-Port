...
void hooks::restoreState(const hooks& saved)
{
	for (auto it = _hooks.begin(); it != _hooks.end(); ) {
		if (saved._hooks.find(it->first) == saved._hooks.end()) {
			if (it->second.getShape())
				_shapes->deleteShape(it->second.getShape());
			it = _hooks.erase(it);
		}
		else
			++it;
	}
	for (auto& sv : saved._hooks) {
		auto it = _hooks.find(sv.first);
		std::shared_ptr<sceneNode> shape;
		if (it == _hooks.end()) {
			char name[12];
			sprintf(name, "H_%u", sv.first);
			shape = _shapes->addShape(sv.second._strong ? sceneNode::nodeType::SPHERE : sceneNode::nodeType::CONE, name);
			it = _hooks.insert(std::make_pair(sv.first, sv.second)).first;
			GLfloat* om = shape->getModelViewMatrix();
			loadIdentity4x4(om);
			float s = sv.second._strong ? _hookSize * 0.1f : _hookSize;
			scaleMatrix4x4(om, s, s, s);
			Vec3f n, vz(0.0f, 0.0f, 1.0f);
			sv.second._tri->getTriangleNormal(sv.second.triangle, n, true);
			float angle = acos(n * vz);
			Vec3f axis = vz ^ n;
			if (fabs(axis[0]) < 1e-5 && fabs(axis[1]) < 1e-5 && fabs(axis[2]) < 1e-5)
				axis[1] = 1.0f;
			axisAngleRotateMatrix4x4(om, axis.xyz, angle);
			translateMatrix4x4(om, sv.second.xyz[0], sv.second.xyz[1], sv.second.xyz[2]);
		}
		else {
			shape = it->second.getShape();
			it->second = sv.second;
			GLfloat* om = shape->getModelViewMatrix();
			om[12] = sv.second.xyz[0];
			om[13] = sv.second.xyz[1];
			om[14] = sv.second.xyz[2];
		}
		it->second.setShape(shape);
		it->second._constraintId = -1;  // re-added by updateHookPhysics after the lattice rebuild
		shape->setColor(it->second._selected ? _selectedColor : _unselectedColor);
	}
	_hookNow = saved._hookNow;
	_selectedHook = saved._selectedHook;
}
