...
public:
	...
	skinCutUndermineTets(const skinCutUndermineTets&) = default;  // copyable for whole-state snapshots (undo)
	skinCutUndermineTets& operator=(const skinCutUndermineTets&) = default;
	~skinCutUndermineTets();
...
public:
	// _deepBed is a static member, so no object copy carries it; an undo
	// snapshot saves and reinstates it through these.
	struct deepBedState { std::unordered_map<int, deepPoint> map; };
	static std::unique_ptr<deepBedState> saveDeepBedState() { auto s = std::make_unique<deepBedState>(); s->map = _deepBed; return s; }
	static void restoreDeepBedState(const deepBedState& s) { _deepBed = s.map; }
	// the deep vertex (flap underside, or bed at the hinge) paired with a top vertex; -1 if none
	static int deepBedVertex(int topVertex) { auto it = _deepBed.find(topVertex); return it == _deepBed.end() ? -1 : it->second.deepMtVertex; }
	// plain-language reason when skinCut() refuses (empty when it failed mid-cut)
	static std::string lastRefusal;
};
