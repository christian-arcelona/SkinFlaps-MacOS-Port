...
private:
	...
	// Same size and alignment as std::atomic<int>, but copyable, so the whole
	// cutter (whose incision bookkeeping is incremental) can be snapshotted for
	// undo.  The cutter's own translation unit is compiled against the original
	// header, so the layout must match it exactly.
	struct copyableCounter {
		int v = 0;
		copyableCounter& operator=(int x) { v = x; return *this; }
	...
	copyableCounter _nSurfaceTets;
	static_assert(sizeof(copyableCounter) == sizeof(std::atomic<int>) && alignof(copyableCounter) == alignof(std::atomic<int>), "cutter layout must match the original header");
