...
void fence::setSpherePos(int postNumber, Vec3f& xyz){
	...
	// The wall's height must follow the fence size, as addPost() sets it
	// (2.5 * _fenceSize, then 0.75 of that on each side); the unit normal
	// alone is a fixed 0.75 model units, within 15% of the scaled height on
	// the face model but eight times too tall on the cleft model (fence
	// size ~0.05).
	vn *= _fenceSize * 2.5f;
