...
#  define SG_GLSL_130 "#version 150 \n"
#else
#  define SG_GLSL_130 "#version 130 \n"
#endif

const GLchar *surgGraphics::skinVertexShader = SG_GLSL_130
	"in vec4 vVertex;"
	"in vec3 vNormal;"
	"in vec3 vTangent;"
	"in vec2 vTexture;"
...
#if defined(__APPLE__)
	// Upgrade GLSL version directive for Mac core profile
	auto patch_version = [](std::string &src){
		const char *v130 = "#version 130";
		const char *v150 = "#version 150";
		size_t pos = src.find(v130);
		if (pos == 0)
			src.replace(0, strlen(v130), v150);
	};
	patch_version(vShd);
	patch_version(fShd);
#endif
...
void surgGraphics::getSkinIncisionLines() {
	...
	else if (_incis.isInitialized())
		_incis.addUpdateIncisions(_incisionLines);  // an undo can remove every incision: drop the stale lines
}
...
void incisionLines::addUpdateIncisions(const std::vector<GLuint> &lines){
	...
	// elementArraySize is an ELEMENT count for glDrawElements (sceneNode::draw);
	// storing the byte size here makes every draw read 4x past the buffer, which
	// shows up as leftover line fragments once an undo shrinks the index set.
	_isn->elementArraySize = (GLsizei)lines.size();
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * lines.size(), lines.empty() ? nullptr : &(lines[0]), GL_STATIC_DRAW);
