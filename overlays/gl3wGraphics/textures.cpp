...
#define LITTLE_ENDIAN_WORD(x) ((void)0)
#endif
#endif
...
int textures::textureExists(const std::string &textureName)
...
bool textures::LoadTGATexture(const char *szFileName, GLenum minFilter, GLenum magFilter, GLenum wrapMode, int& width, int& height)
{
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    // Use a valid internal format for core profile
    GLint internal_fmt = (nComponents == GL_RGBA) ? GL_RGBA : GL_RGB;
    glTexImage2D(GL_TEXTURE_2D, 0, internal_fmt, nWidth, nHeight, 0,
