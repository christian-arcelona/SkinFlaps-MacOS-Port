...
void gl3wGraphics::setViewport(int x, int y, int xSize, int ySize)
{
	assert(x==0 && y==0);
	if (xSize < 1) xSize = 1;
	if (ySize < 1) ySize = 1;
...
void gl3wGraphics::mouseButtonEvent(int screenX, int screenY, int button, bool dragging)
{
	if (_xSize < 1 || _ySize < 1)
		return;
	const int clampedX = std::max(0, std::min(screenX, _xSize - 1));
	const int clampedY = std::max(0, std::min(screenY, _ySize - 1));
	if(dragging)	{
		_dragging=true;
		if( button<1)	{	// leftMouse
            ...
            (2.0f*clampedX - _xSize)    / _xSize,
            (_ySize - 2.0f*clampedY)    / _ySize);
			_tBall.add_quats(spin_quat, _rotQuat, _rotQuat);
		}
		else if (button > 1) {	// rightMouse
			if (!mouseWheelZoom)
				_glM.changeZoom((float)(_lastY - clampedY) / _ySize);
		}
		else	{	// middleMouse
			_glM.setPan((float)(_lastX-clampedX)/_xSize,(float)(clampedY-_lastY)/_ySize);
		}
	}
	_lastX = clampedX;
	_lastY = clampedY;
...
{
	if (_xSize < 1 || _ySize < 1) {
		lineStartPosition[0] = lineStartPosition[1] = lineStartPosition[2] = 0.0f;
		lineDirection[0] = lineDirection[1] = 0.0f;
		lineDirection[2] = -1.0f;
		return;
	}
	float sc[3];
...
bool gl3wGraphics::pick(int x, int y, std::string &name, float (&position)[3], int &triangle, bool excludeShapes, bool excludeStatic)
{
	if (_xSize < 1 || _ySize < 1)
		return false;
	const int clampedX = std::max(0, std::min(x, _xSize - 1));
	const int clampedY = std::max(0, std::min(y, _ySize - 1));
	bool picked=false;
	triangle = -1;
	name = "";
	...
	_glM.getCameraData(zCenter,height,verticalAngle,screenAspect);
	height = -zCenter*2.0f*tanf(verticalAngle*0.5f);
	sc[0] = (-0.5f+(float)clampedX/_xSize)*height*screenAspect;
	sc[1] = (0.5f-(float)clampedY/_ySize)*height;
	sc[2] = zCenter;
