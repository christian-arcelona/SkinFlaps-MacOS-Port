...
public:
	...
	void mouseButtonEvent(int screenX, int screenY, int button, bool dragging);
	void mouseWheelEvent(float delta) { _glM.changeZoom(delta * 0.04f); }
	void setMouseWheelZoom(bool wheelZoom) { mouseWheelZoom = wheelZoom; }
	bool pick(int x, int y, std::string &name, float (&position)[3], int &triangle, bool excludeShapes=false, bool excludeStatic = true);
	...
	void setViewport(int x, int y, int xSize, int ySize);
...
private:
	trackball _tBall;
	float _rotQuat[4];
	int _xSize,_ySize,_lastX,_lastY;
	int _staticCount;
	...
	GLmatrices _glM;
	GLuint  texture;
	static bool mouseWheelZoom;
	static float mouseWheelLevel;
};
