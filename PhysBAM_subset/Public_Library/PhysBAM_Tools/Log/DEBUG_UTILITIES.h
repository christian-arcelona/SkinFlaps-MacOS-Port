...
// Optional message arguments are accepted but ignored: '##__VA_ARGS__' comma
// swallowing is not portable across GCC/Clang, and the zero-argument overloads
// carry the function/file/line context that matters.

#define PHYSBAM_NOT_IMPLEMENTED(...) \
    PhysBAM::DEBUG_UTILITIES::Not_Implemented(PHYSBAM_DEBUG_FUNCTION_NAME,__FILE__,__LINE__)

#define PHYSBAM_FATAL_ERROR(...) \
    PhysBAM::DEBUG_UTILITIES::Fatal_Error(PHYSBAM_DEBUG_FUNCTION_NAME,__FILE__,__LINE__)

#define PHYSBAM_ASSERT(condition, ...) \
    if(condition){}else PhysBAM::DEBUG_UTILITIES::Assertion_Failed(PHYSBAM_DEBUG_FUNCTION_NAME,__FILE__,__LINE__,#condition)
