...
// Forward declare TIMER to satisfy pointer members regardless of include order
class TIMER;
...
struct log_null_class {
    ...
    template<class T> void precision(const T&){};
    template<class A,class B> void Copy_Log_To_File(const A&,const B&,const int threadid=1){}
};
extern log_null_class cout;
extern log_null_class cerr;
template<class T> inline void Time(const T&,const int threadid=1){}
template<class A,class B> inline void Stat(const A&,const B&,const int threadid=1){}
inline void Stop_Time(const int threadid=1){}
inline void Finish_Logging(const int threadid=1){}
inline void filecout(const std::string& buffer,const int threadid=1){}
inline bool Check_Log_Initialized(){return false;}
template<class A,class B,class C,class D> inline void Initialize_Logging(const A&,const B&,const C&,const D&,const int nthreads=1){}
template<class A,class B,class C> inline void Initialize_Logging(const A&,const B&,const C&){}
...
namespace{
    #ifndef PHYSBAM_DISABLE_LOG_GLOBALS
